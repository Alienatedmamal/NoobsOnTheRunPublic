...
namespace Oxide.Plugins 
{
    ...
    //  Done
    //  Fix for online/offline player list ordering.
    ...
    class PermissionsManager : RustPlugin
    {
        ...
        void GetPlugs(BasePlayer player)
        {
            ...
            foreach (var entry in plugins.GetAll())
            {
                ...
                var str = entry.ToString().Replace("Oxide.Plugins.", "").ToLower();

                foreach (var perm in permission.GetPermissions().ToList().Where(perm => perm.ToLower().Contains($"{str}") && !(config.BlockList.ToLower().Split(',').ToList().Contains($"{str}"))))
        ...
        object[] PermsCheck(BasePlayer player, string group, string info)
        {
            ...
            {
                var userData = permission.GetUserData(path.subject.UserIDString);
        ...
        void PMMainUI(BasePlayer player, bool group, int page)
        {
            ...
            { 
                foreach (var plyr in BasePlayer.allPlayerList.OrderBy(x => x.displayName).OrderBy(x => x?.net?.connection == null))
                {
                    pos1++;
                    quantity++;
                    if (pos1 > 0 && pos1 < 21)
                    {
                        elements.Add(new CuiButton { Button = { Command = $"PMSelected player {plyr.userID}", Color = config.ButtonColour }, RectTransform = { AnchorMin = $"0.3 {bottom}", AnchorMax = $"0.7 {top}" }, Text = { Text = StripSlashes(plyr.displayName), FontSize = 11, Align = TextAnchor.MiddleCenter, Color = plyr?.net?.connection == null ? "1 1 1 1" : "0.2 1 0.2 1" } }, mainName);
                        top = top - 0.025f;
                        bottom = bottom - 0.025f;
                    }
                }
        ...
        void PMDataUI(BasePlayer player)
        {
            ...
            elements.Add(new CuiLabel { Text = { Text = "Global remove.", FontSize = 13, Align = TextAnchor.MiddleCenter }, RectTransform = { AnchorMin = "0.1 0.17", AnchorMax = $"0.898 0.20" }, }, mainName);
        ...
        void PlugsUI(BasePlayer player, string msg, string group, int page)
        {
            ...
            if (group == "false") 
                elements.Add(new CuiButton { Button = { Command = $"PMRevokeAllPlayer", Color = config.ButtonColour }, RectTransform = { AnchorMin = "0.4 0.1", AnchorMax = "0.6 0.14" }, Text = { Text = lang.GetMessage("RevokeAll", this), FontSize = 11, Align = TextAnchor.MiddleCenter } }, mainName);
        ...
        void ViewGroupsUI(BasePlayer player, string msg, int page)
        {
            ...
            elements.Add(new CuiButton { Button = { Command = $"PMRemoveFromAllGroups {msg} {page}", Color = config.ButtonColour }, RectTransform = { AnchorMin = "0.4 0.1", AnchorMax = "0.6 0.14" }, Text = { Text = lang.GetMessage("RemoveFromAll", this), FontSize = 11, Align = TextAnchor.MiddleCenter } }, mainName);
        ...
        //// ADDED
        [ConsoleCommand("PMRevokeAllPlayer")]
        private void PMRevokeAllPlayer(ConsoleSystem.Arg arg)
        {
            var player = arg?.Connection?.player as BasePlayer;
        ...
        private void PMRevoke(ConsoleSystem.Arg arg)
        {
            ...
            if (confirmation == "true")
            {
                foreach (var perm in permission.GetUserPermissions(path.subject.UserIDString))
                    permission.RevokeUserPermission(path.subject.UserIDString, perm);
        ...
        [ConsoleCommand("PMRemoveFromAllGroups")]
        private void PMRemoveFromAllGroups(ConsoleSystem.Arg arg) 
        {
            var player = arg?.Connection?.player as BasePlayer;

            var elements1 = new CuiElementContainer();
            var mainName1 = elements1.Add(new CuiPanel { Image = { Color = "0.1 0.1 0.1 0.8" }, RectTransform = { AnchorMin = "0.4 0.42", AnchorMax = "0.6 0.48" }, CursorEnabled = true, FadeOut = 0.1f }, "Overlay", "PMConfirmUI");
            elements1.Add(new CuiButton { Button = { Command = $"PMRemove true {arg.Args[0]}, {arg.Args[1]}", Color = config.ButtonColour }, RectTransform = { AnchorMin = "0.1 0.2", AnchorMax = "0.4 0.8" }, Text = { Text = lang.GetMessage("confirm", this), FontSize = 14, Align = TextAnchor.MiddleCenter }, }, mainName1);
        ...
        private void PMRemove(ConsoleSystem.Arg arg)
        {
            ...
            if (confirmation == "true") 
            {
                ...
                DestroyMenu(arg.Player(), false);
                ViewGroupsUI(arg.Player(), arg.Args[1], Convert.ToInt16(arg.Args[2]));
            }
            else
                CuiHelper.DestroyUi(arg.Player(), "PMConfirmUI");
        }
        ...
        private void WipeSQL()
        {
            ...
            Puts("Permissions Manager MySQL backup was dropped.");
        }
        ...
        void DoLocal(bool save, bool groups)
        {
            ...
            if (save)
            {
                ...
                if (groups)
                {
                    storedData.PermsData = storedData.PermsData.Where(x => x.Value.player == 1).ToDictionary(x => x.Key, x => x.Value);
                    foreach (var group in permission.GetGroups())
                    {
                        perms = new List<string>();
                ...
                {
                    var userData = ProtoStorage.Load<Dictionary<string, Core.Libraries.UserData>>(new string[] { "oxide.users" }); 
                    userData = userData.Where(x => x.Value.Perms.Count > 0).ToDictionary(x => x.Key, x => x.Value);
                    storedData.PermsData = storedData.PermsData.Where(x => x.Value.player == 0).ToDictionary(x => x.Key, x => x.Value);
            ...
            else
            {
                ...
                {
                    if (record.Value.player == 0 && groups)
                    {
                    ...
                    else if (record.Value.player != 0 && !groups) 
                    {
                        foreach (var perm in record.Value.Perms)
